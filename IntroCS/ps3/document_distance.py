...
def get_frequencies(input_iterable):
    ...
    for i in range(len(input_iterable)):
        if input_iterable[i] not in freq_dict:
            freq_dict[input_iterable[i]] = 1
        else:
            freq_dict[input_iterable[i]] += 1
    return freq_dict
...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    for i in union_list:
        ...
        freq2_diff = 0
        if i in freq_dict1:
            freq1_diff = freq_dict1[i]
        else:
            freq1_diff = 0
        if i in freq_dict2:
            freq2_diff = freq_dict2[i]
        else:
            freq2_diff = 0
        freq_diff_sum += abs(freq1_diff - freq2_diff)
        freq_total_sum += (freq1_diff + freq2_diff)
    return round(1 - (freq_diff_sum / freq_total_sum), 2)
...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    for k in combined_dict:
        if combined_dict[k] > largest_value:
            largest_value = combined_dict[k]
    most_frequent = []
    for k in combined_dict:
        if combined_dict[k] == largest_value:
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    ...
    text_list = text_to_list(file_text)
    total_word_count = len(text_list)
    tf_dict = {}
    freq_dict = get_frequencies(text_list)
    for i in text_list:
        tf_dict[i] = (freq_dict[i] / total_word_count)
    return tf_dict
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    """
    no_documents = len(file_paths)
    idf_dict  = {}
    document_dict = {}
    for i in range(no_documents):
        file_text = load_file(file_paths[i])
        text_list = text_to_list(file_text)
        document_dict[i] = get_frequencies(text_list)
    for i in range(no_documents):
        current_dict = document_dict[i]
        for j in current_dict:
            if j not in idf_dict:
                ...
                idf_dict[j] += 1
    for i in idf_dict:
        idf_dict[i] = abs(math.log10(idf_dict[i] / no_documents))
    return idf_dict
...
def get_tfidf(tf_file_path, idf_file_paths):
    ...
    tf_idf_list = []
    for i in tf_dict:
        tf_idf_list.append((i, (tf_dict[i] * idf_dict[i])))
    return sorted(tf_idf_list, key=lambda tuple: tuple[1])
